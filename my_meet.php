      ...
      <div class="top-products-area clearfix py-3">
        <div class="container-fluid">
          <div class="section-heading d-flex align-items-center justify-content-between">
            <h6 class="ml-1">Active Meets</h6><a class="btn btn-danger btn-sm" href="#"></a>
          </div>
          <div class="row g-3">
			...
			<?php
				  ...
				  $sql = "SELECT * FROM classrooms cr, meet m, classroom_student cs WHERE cr.cr_id=m.cr_id AND cr.cr_id=cs.cr_id AND cs.euid=$user_id AND m.meet_status=1 ORDER BY m.meet_id DESC";
						...
						echo"<div class='col-12 col-md-4 col-lg-3'>
						  <div class='card top-product-card'>
							<div class='card-body'><span class='badge badge-success'>";
							   if($row['meet_status']==1){echo "Active";}
							   else if ($row['meet_status']==2){echo "Suspended";}						
							   else if($row['meet_status']==0){echo "Deleted";}			
							   else {echo "None";}
							echo"</span>
							...
							<a class='product-title d-block' href='#'>".$row['meet_name']."</a>
							<p class='sale-price'> Classroom: ".$row['cr_name']."<span></span></p>
							<p class='sale-price'> Date: ".date('d-m-Y', strtotime($row['meet_date']))." ".date('g:h a', strtotime($row['meet_time']))."<span></span></p>
							  <div class='product-rating1'>
							  </div>
							  <a class='btn btn-success btn-sm add2cart-notify' target='self' href='".$row['meet_link']."'><i class='lni lni-eye'></i> Join Meet</a>
							</div>
						  </div>
						</div>";
					 }
