...
while ($compResult = $compQuery->fetch_assoc()) {
 ...
 if (isset($_POST['submitbycr'])){
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		$sql = "SELECT * FROM Assignment ass, classrooms cr WHERE cr.cr_id=ass.cr_id AND ass.cr_id=$cr_id AND ass.ass_status=1 ORDER BY ass.ass_id DESC";
	}
}
else if (isset($_POST['submitbyname'])){
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		$ass_name = test_input($_POST["txtname"]);
		$sql = "SELECT * FROM Assignment ass, classrooms cr WHERE cr.cr_id=ass.cr_id AND ass.ass_name='$ass_name' AND ass.ass_status=1 ORDER BY ass.ass_id DESC";
	}
}
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if($status==3){
			$sql = "SELECT * FROM Assignment ass, classrooms cr WHERE cr.cr_id=ass.cr_id ORDER BY ass.ass_id DESC";
		}
		else{
		$sql = "SELECT * FROM Assignment ass, classrooms cr WHERE cr.cr_id=ass.cr_id AND ass.ass_status=$status ORDER BY ass.ass_id DESC";
		}
	}
}
else{
	$sql = "SELECT * FROM Assignment ass, classrooms cr WHERE cr.cr_id=ass.cr_id AND ass.ass_status=1 ORDER BY ass.ass_id DESC";
}
