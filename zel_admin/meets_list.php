...
while ($compResult = $compQuery->fetch_assoc()) {
 ...
 if (isset($_POST['submitbycr'])){
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		$sql = "SELECT * FROM meet m, classrooms cr WHERE cr.cr_id=m.cr_id AND m.cr_id=$cr_id AND m.meet_status=1 ORDER BY m.meet_id DESC";
	}
}
else if (isset($_POST['submitbyname'])){
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		$meet_name = test_input($_POST["txtname"]);
		$sql = "SELECT * FROM meet m, classrooms cr WHERE cr.cr_id=m.cr_id AND m.meet_name='$meet_name' AND m.meet_status=1 ORDER BY m.meet_id DESC";
	}
}
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if($status==3){
			$sql = "SELECT * FROM meet m, classrooms cr WHERE cr.cr_id=m.cr_id ORDER BY m.meet_id DESC";
		}
		else{
		$sql = "SELECT * FROM meet m, classrooms cr WHERE cr.cr_id=m.cr_id AND m.meet_status=$status ORDER BY m.meet_id DESC";
		}
	}
}
else{
	$sql = "SELECT * FROM meet m, classrooms cr WHERE cr.cr_id=m.cr_id AND m.meet_status=1 ORDER BY m.meet_id DESC";
}
