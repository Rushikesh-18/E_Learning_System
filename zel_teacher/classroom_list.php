...
while ($compResult = $compQuery->fetch_assoc()) {
 ...
 if (isset($_POST['submitbycourse'])){
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		$sql = "SELECT * FROM classrooms cr, course c WHERE c.cid = cr.cid AND cr.cr_status=1 AND cr.cid=$cid AND cr.uid=$user_id ORDER BY cr.cr_id DESC";
	}
}
else if (isset($_POST['submitbyname'])){
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		$cr_name = test_input($_POST["txtname"]);
		$sql = "SELECT * FROM classrooms cr, course c WHERE c.cid = cr.cid AND cr.cr_status=1 AND cr.cr_name='$cr_name' AND cr.uid=$user_id ORDER BY cr.cr_id DESC";
	}
}
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if($status==3){
			$sql = "SELECT * FROM classrooms cr, course c WHERE c.cid = cr.cid AND cr.uid=$user_id ORDER BY cr.cr_id DESC";
		}
		else{
		$sql = "SELECT * FROM classrooms cr, course c WHERE c.cid = cr.cid AND cr.cr_status=$status AND cr.uid=$user_id ORDER BY cr.cr_id DESC";
		}
	}
}
else{
	$sql = "SELECT * FROM classrooms cr, course c WHERE c.cid=cr.cid AND cr.cr_status=1 AND cr.uid=$user_id ORDER BY cr.cr_id DESC";
}
