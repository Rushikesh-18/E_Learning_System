...
while ($compResult = $compQuery->fetch_assoc()) {
 ...
 if (isset($_POST['submitbycr'])){
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		$sql = "SELECT * FROM meet m, classrooms cr WHERE cr.cr_id=m.cr_id AND m.cr_id=$cr_id AND m.meet_status=1 AND cr.uid=$user_id ORDER BY m.meet_id DESC";
	}
}
else if (isset($_POST['submitbyname'])){
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		$meet_name = test_input($_POST["txtname"]);
		$sql = "SELECT * FROM meet m, classrooms cr WHERE cr.cr_id=m.cr_id AND m.meet_name='$meet_name' AND m.meet_status=1 AND cr.uid=$user_id ORDER BY m.meet_id DESC";
	}
}
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if($status==3){
			$sql = "SELECT * FROM meet m, classrooms cr WHERE cr.cr_id=m.cr_id AND cr.uid=$user_id ORDER BY m.meet_id DESC";
		}
		else{
		$sql = "SELECT * FROM meet m, classrooms cr WHERE cr.cr_id=m.cr_id AND m.meet_status=$status AND cr.uid=$user_id ORDER BY m.meet_id DESC";
		}
	}
}
else{
	$sql = "SELECT * FROM meet m, classrooms cr WHERE cr.cr_id=m.cr_id AND m.meet_status=1 AND cr.uid=$user_id ORDER BY m.meet_id DESC";
}
...
<div class="row">
	...
	<div class="panel panel-info">
		  <div class="panel-heading" align="center">Search Meets</div>
		  <div class="panel-body">
			<form class="form-inline" data-toggle="validator" name="sub" id="sub" role="form" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
				...
				<div class="col-md-4" >
					<div class="form-group">
					<input class="form-control auto" type="text" id="txtname" name= "txtname" placeholder="Meet Name" value="<?php echo $euname?>">
					</div>
					<div class="form-group" align="center">
...
<div class="col-md-12">
<div class="panel panel-info">
      <div class="panel-heading" align="center">Meet List <a style="display:none;" class="pull-right" align="right" href="#" onclick=printbill(); >Print List</a></div>
      <div class="panel-body">
        ...
        // output data of each row
          ...
          while($row = $result->fetch_assoc()) {
			...
			echo "<td>".$row['meet_id']."</td>";
              ...
              echo "<td>".$row['meet_desc']."</td>";
			   echo "<td>".date( 'd/m/Y', strtotime($row['meet_regdate']))."</td>";
			   echo "<td>".date( 'd/m/Y', strtotime($row['meet_date']))."</td>";
			   echo "<td>".date( 'g:h a', strtotime($row['meet_time']))."</td>";			   
			   echo"<td><u><a href='".$row['meet_link']."' target='self' >Meet Link</a></u></td>";
			   if($row['meet_status']==1){echo "<td class='success'>Active</td>";}
			   else if ($row['meet_status']==2){echo "<td class='warning'>Suspended</td>";}						
			   else if ($row['meet_status']==0){echo "<td class='danger'>Deleted</td>";}						
			...
			<li><a type='button' id='deletebtn' onclick='delete_meet(".$row['meet_id'].")'>Delete</a></li>
			</ul>
			</div></td>";
			echo "</tr>";
         }
